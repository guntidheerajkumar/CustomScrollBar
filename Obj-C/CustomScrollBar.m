...
static const CGFloat kCustomScrollBarTrackWidth      = 2.0f;     // The default width of the scrollable space indicator
static const CGFloat kCustomScrollBarHandleWidth     = 4.0f;     // The default width of the handle control
static const CGFloat kCustomScrollBarEdgeInset       = 7.5f;     // The distance from the edge of the view to the center of the track
static const CGFloat kCustomScrollBarHandleMinHeight = 64.0f;    // The minimum usable size to which the handle can shrink
static const CGFloat kCustomScrollBarWidth           = 44.0f;    // The width of this control (44 is minimum recommended tapping space)
static const CGFloat kCustomScrollBarVerticalPadding = 10.0f;    // The default padding at the top and bottom of the view
static const CGFloat kCustomScrollBarMinimumContentScale = 5.0f; // The minimum scale of the content view before showing the scroll view is necessary

/************************************************************************/

// A struct to hold the scroll view's previous state before this bar was applied
struct CustomScrollBarScrollViewState {
    BOOL showsVerticalScrollIndicator;
};
typedef struct CustomScrollBarScrollViewState CustomScrollBarScrollViewState;
...
@interface CustomScrollBar () <UIGestureRecognizerDelegate> {
    CustomScrollBarScrollViewState _scrollViewState;
}
...
- (void)setUpViews;
- (void)configureViewsForStyle:(CustomScrollBarStyle)style;
...
- (void)setUpInitialProperties
{
    _trackWidth  = kCustomScrollBarTrackWidth;
    _handleWidth = kCustomScrollBarHandleWidth;
    _edgeInset   = kCustomScrollBarEdgeInset;
    _handleMinimiumHeight = kCustomScrollBarHandleMinHeight;
    _minimumContentHeightScale = kCustomScrollBarMinimumContentScale;
    _verticalInset = UIEdgeInsetsMake(kCustomScrollBarVerticalPadding, 0.0f, kCustomScrollBarVerticalPadding, 0.0f);
...
- (void)setUpViews
{
    ...
    self.trackView = [[UIImageView alloc] initWithImage:[CustomScrollBar verticalCapsuleImageWithWidth:self.trackWidth]];
    [self addSubview:self.trackView];
    
    // Add the handle view
    self.handleView = [[UIImageView alloc] initWithImage:[CustomScrollBar verticalCapsuleImageWithWidth:self.handleWidth]];
...
- (void)configureViewsForStyle:(CustomScrollBarStyle)style
{
    BOOL dark = (style == CustomScrollBarStyleDark);
...
- (void)layoutInScrollView
{
    ...
    CGFloat halfWidth      = (kCustomScrollBarWidth * 0.5f);
    ...
    frame.size.width = kCustomScrollBarWidth;
    frame.size.height = height;
    frame.origin.x = scrollViewFrame.size.width - (_edgeInset + halfWidth);
    frame.origin.x = MIN(frame.origin.x, scrollViewFrame.size.width - kCustomScrollBarWidth);
...
- (void)setStyle:(CustomScrollBarStyle)style
{
    _style = style;
